				...
				for(var pi = 0, pl = parts.length, part; pi < pl; pi++)
				{
					...
					if (partSymbol)
					{
					  if (isDeclarationPart)
					    result.push(SPAN+new Link().toSrc(partSymbol.srcFile, partSymbol.lineNumber).withText(partName)+
					      "<span class='doclink'>" + new Link().toSymbol(partSymbol.alias).withText("^") + "</span></span>");
					  else
